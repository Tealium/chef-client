...
return unless node['lsb']['codename'].eql?('trusty')

if ::File.executable?(node['chef_client']['bin'])
  client_bin = node['chef_client']['bin']
  # search for the bin in some sane paths
elsif Chef::Client.const_defined?('SANE_PATHS') && (chef_in_sane_path = Chef::Client::SANE_PATHS.map do |p|
  p = "#{p}/chef-client"; p if ::File.executable?(p)
end .compact.first) && chef_in_sane_path
